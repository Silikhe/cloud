<main>
		<section id="hero_in" class="courses">
			<div class="wrapper">
				<div class="container">
					<h1 class="fadeInUp"><span></span>Online course detail</h1>
				</div>
			</div>
		</section>
		<!--/hero_in-->

		<div class="bg_color_1">
			<nav class="secondary_nav sticky_horizontal">
				<div class="container">
					<ul class="clearfix">
						<li><a href="#description" class="active">Description</a></li>
						<li><a href="#lessons">Programme</a></li>
						<li><a href="#teachers">Teachers</a></li>
					</ul>
				</div>
			</nav>
			<div class="container margin_60_35">
				<div class="row">
					<div class="col-lg-8">
						<section id="description">
							<div class="box_highlight">
								<ul class="additional_info">
									<li><i class="pe-7s-timer"></i>Course Duration<strong>14 Weeks</strong></li>
									<li><i class="pe-7s-date"></i>Course Start<strong>10 Sept. 2021</strong></li>
									<li><i class="pe-7s-wallet"></i>Course Fee<strong>KES 41000</strong></li>
								</ul>
							</div>
							<!-- /box_highlight -->
							<h2>Description</h2>
							<p>Per consequat adolescens ex, cu nibh commune temporibus vim, ad sumo viris <strong>eloquentiam sed</strong>. Mea appareat omittantur eloquentiam ad, nam ei quas oportere democritum. Prima causae admodum id est, ei timeam inimicus sed. Sit an meis aliquam, cetero inermis vel ut. An sit illum euismod facilisis, tamquam <strong>vulputate</strong> pertinacia eum at.</p>
							<p>Mea appareat omittantur eloquentiam ad, nam ei quas oportere democritum. Prima causae admodum id est, ei timeam inimicus sed. Sit an meis aliquam, cetero inermis vel ut. An sit illum euismod facilisis, tamquam vulputate pertinacia eum at.</p>
							<div class="row">
								<div class="col-lg-6">
									<ul class="bullets">
										<li>Dolorem mediocritatem</li>
										<li>Mea appareat</li>
										<li>Prima causae</li>
										<li>Singulis indoctum</li>
									</ul>
								</div>
								<div class="col-lg-6">
									<ul class="bullets">
										<li>Timeam inimicus</li>
										<li>Oportere democritum</li>
										<li>Cetero inermis</li>
										<li>Pertinacia eum</li>
									</ul>
								</div>
							</div>
							<!-- /row -->
						</section>
						<!-- /section -->
						
						<section id="lessons">
							<div class="intro_title">
								<h2>Programme</h2>
							</div>
							<div id="accordion_lessons" role="tablist" class="add_bottom_45">
								<div class="card">
									<div class="card-header" role="tab" id="headingOne">
										<h5 class="mb-0">
											<a data-bs-toggle="collapse" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne"><i class="indicator ti-minus"></i> Week 1</a>
										</h5>
									</div>

									<div id="collapseOne" class="collapse show" role="tabpanel" aria-labelledby="headingOne">
										<div class="card-body">
											<p>Lorem ipsum dolor sit amet, pro ea <strong>mediocrem sapientem</strong>. Et his copiosae vivendum, corpora contentiones vel ei. Ne etiam graecis vis. Nec omnis alienum no, in quas corpora inimicus his, nec pertinacia disputando voluptatibus ei. In eam suas perpetua accusamus. Ad sit virtute rationibus efficiantur, sed hinc explicari ea.</p>
											<h6>Week 1: HTML, CSS, and Git</h6>
											<div class="list_lessons_2">
												<ul>
													<li>Command-line<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Text editors<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Version control and Git<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Debugging CSS and HTML<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Responsive design and media queries<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
												</ul>
											</div>
											<!-- <h6>Semester 2</h6>
											<div class="list_lessons_2">
												<ul>
													<li>Health Science<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Health and Social Care<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>History<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Healthcare Science<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Audiology<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
												</ul>
											</div> -->
										</div>
									</div>
								</div>
								<!-- /card -->
								<div class="card">
									<div class="card-header" role="tab" id="headingTwo">
										<h5 class="mb-0">
											<a data-bs-toggle="collapse" data-bs-toggle="collapse" href="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
												<i class="indicator ti-plus"></i>Week 3
											</a>
										</h5>
									</div>
									<div id="collapseTwo" class="collapse" role="tabpanel" aria-labelledby="headingTwo">
										<div class="card-body">
											<p>Lorem ipsum dolor sit amet, pro ea <strong>mediocrem sapientem</strong>. Et his copiosae vivendum, corpora contentiones vel ei. Ne etiam graecis vis. Nec omnis alienum no, in quas corpora inimicus his, nec pertinacia disputando voluptatibus ei. In eam suas perpetua accusamus. Ad sit virtute rationibus efficiantur, sed hinc explicari ea.</p>
											<h6>JavaScript Fundamentals</h6>
											<div class="list_lessons_2">
												<ul>
													<li>Introduction to JavaScript<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>JavaScript console<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Fundamental JavaScript<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Business vs User interface logic<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Looping<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
												</ul>
											</div>
											<h6>Outcome</h6>
											<div class="list_lessons_2">
												<ul>
													<li>Write and use custom javascript functions<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Understand the conventions used in naming variables<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Manipulate numbers with methods<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Healthcare Science<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Audiology<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
												</ul>
											</div>
										</div>
									</div>
								</div>
								<!-- /card -->
								<div class="card">
									<div class="card-header" role="tab" id="headingThree">
										<h5 class="mb-0">
											<a data-bs-toggle="collapse" data-bs-toggle="collapse" href="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
												<i class="indicator ti-plus"></i>Year 3
											</a>
										</h5>
									</div>
									<div id="collapseThree" class="collapse" role="tabpanel" aria-labelledby="headingThree">
										<div class="card-body">
											<p>Lorem ipsum dolor sit amet, pro ea <strong>mediocrem sapientem</strong>. Et his copiosae vivendum, corpora contentiones vel ei. Ne etiam graecis vis. Nec omnis alienum no, in quas corpora inimicus his, nec pertinacia disputando voluptatibus ei. In eam suas perpetua accusamus. Ad sit virtute rationibus efficiantur, sed hinc explicari ea.</p>
											<h6>Semester 1</h6>
											<div class="list_lessons_2">
												<ul>
													<li>Health Science<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Health and Social Care<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>History<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Healthcare Science<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Audiology<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
												</ul>
											</div>
											<h6>Semester 2</h6>
											<div class="list_lessons_2">
												<ul>
													<li>Health Science<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Health and Social Care<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>History<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Healthcare Science<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
													<li>Audiology<span><a href="#0"><i class="icon_download"></i> PDF</a></span></li>
												</ul>
											</div>
										</div>
									</div>
								</div>
								<!-- /card -->								
							</div>
							<!-- /accordion -->
						</section>
						<!-- /section -->
						<section id="teachers">
							<div class="intro_title">
								<h2>Teachers</h2>
							</div>
							<p>Zril causae ancillae sit ea. Dicam veritus mediocritatem sea ex, nec id agam eius. Te pri facete latine salutandi, scripta mediocrem et sed, cum ne mundi vulputate. Ne his sint graeco detraxit, posse exerci volutpat has in.</p>
							<div class="row add_top_20 add_bottom_30">
								<div class="col-lg-6">
									<ul class="list_teachers">
										<li>
											<a href="teacher-detail.html">
												<figure><img src="<?= Yii::$app->request->baseUrl ?>/theme/udema/img/teacher_1_thumb.jpg" alt=""></figure>
												<h5>Tomas Hegel</h5>
												<p>Mathematics</p><i class="pe-7s-angle-right-circle"></i></a>
										</li>
										<li>
											<a href="teacher-detail.html">
												<figure><img src="<?= Yii::$app->request->baseUrl ?>/theme/udema/img/teacher_2_thumb.jpg" alt=""></figure>
												<h5>Tomas Hegel</h5>
												<p>Mathematics</p><i class="pe-7s-angle-right-circle"></i></a>
										</li>
										<li>
											<a href="teacher-detail.html">
												<figure><img src="<?= Yii::$app->request->baseUrl ?>/theme/udema/img/teacher_3_thumb.jpg" alt=""></figure>
												<h5>Tomas Hegel</h5>
												<p>Mathematics</p><i class="pe-7s-angle-right-circle"></i></a>
										</li>
									</ul>
								</div>
								<div class="col-lg-6">
									<ul class="list_teachers">
										<li>
											<a href="teacher-detail.html">
												<figure><img src="<?= Yii::$app->request->baseUrl ?>/theme/udema/img/teacher_4_thumb.jpg" alt=""></figure>
												<h5>Tomas Hegel</h5>
												<p>Mathematics</p><i class="pe-7s-angle-right-circle"></i></a>
										</li>
										<li>
											<a href="teacher-detail.html">
												<figure><img src="<?= Yii::$app->request->baseUrl ?>/theme/udema/img/teacher_5_thumb.jpg" alt=""></figure>
												<h5>Tomas Hegel</h5>
												<p>Mathematics</p><i class="pe-7s-angle-right-circle"></i></a>
										</li>
										<li>
											<a href="teacher-detail.html">
												<figure><img src="<?= Yii::$app->request->baseUrl ?>/theme/udema/img/teacher_6_thumb.jpg" alt=""></figure>
												<h5>Tomas Hegel</h5>
												<p>Mathematics</p><i class="pe-7s-angle-right-circle"></i></a>
										</li>
									</ul>
								</div>
							</div>
							<!-- /row -->
						</section>
						<!-- /section -->
					</div>
					<!-- /col -->
					
					<aside class="col-lg-4" id="sidebar">
						<div class="box_detail">
							<h4>Enquire now</h4>
							<p class="nopadding">Ex quem dicta delicata usu, zril vocibus maiestatis in qui.</p>
							<div id="message-contact"></div>
							<form method="post" action="assets/contact.php" id="contactform" autocomplete="off">
								<div class="row">
									<div class="col-xl-6 col-lg-12 col-sm-6">
										<span class="input">
											<input class="input_field" type="text" id="name_contact" name="name_contact">
											<label class="input_label">
												<span class="input__label-content">Your Name</span>
											</label>
										</span>
									</div>
									<div class="col-xl-6 col-lg-12 col-sm-6">
										<span class="input">
											<input class="input_field" type="text" id="lastname_contact" name="lastname_contact">
											<label class="input_label">
												<span class="input__label-content">Last name</span>
											</label>
										</span>
									</div>
								</div>
								<!-- /row -->
								<div class="row">
									<div class="col-xl-6 col-lg-12 col-sm-6">
										<span class="input">
											<input class="input_field" type="email" id="email_contact" name="email_contact">
											<label class="input_label">
												<span class="input__label-content">Your email</span>
											</label>
										</span>
									</div>
									<div class="col-xl-6 col-lg-12 col-sm-6">
										<span class="input">
											<input class="input_field" type="text" id="phone_contact" name="phone_contact">
											<label class="input_label">
												<span class="input__label-content">Your telephone</span>
											</label>
										</span>
									</div>
								</div>
								<!-- /row -->
								<span class="input">
										<textarea class="input_field" id="message_contact" name="message_contact" style="height:120px;"></textarea>
										<label class="input_label">
											<span class="input__label-content">Your message</span>
										</label>
								</span>
								<span class="input">
										<input class="input_field" type="text" id="verify_contact" name="verify_contact">
										<label class="input_label">
										<span class="input__label-content">Are you human? 3 + 1 =</span>
										</label>
								</span>
								<div style="position:relative;" class="mt-4"><input type="submit" value="Enquire Now" class="btn_1 full-width" id="submit-contact"></div>
							</form>
							</div>
					</aside>
				</div>
				<!-- /row -->
			</div>
			<!-- /container -->
		</div>
		<!-- /bg_color_1 -->
	</main>